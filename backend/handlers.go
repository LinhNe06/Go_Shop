...
func getProduct(c *gin.Context) {
	var p Product
	err := db.QueryRow(`SELECT id, name, price, description, image, created_at FROM products WHERE id = $1`, c.Param("id")).
...
func createProduct(c *gin.Context) {
	...
	var p Product
	err = db.QueryRow(`INSERT INTO products (name, price, description, image)
		VALUES ($1, $2, $3, $4) RETURNING id, name, price, description, image, created_at`,
...
func updateProduct(c *gin.Context) {
	var current Product
	err := db.QueryRow(`SELECT id, name, price, description, image FROM products WHERE id = $1`, c.Param("id")).
	...
	if value := c.PostForm("price"); value != "" {
		price, err := strconv.ParseInt(value, 10, 64)
		if err != nil || price < 0 {
			c.JSON(http.StatusBadRequest, gin.H{"message": "price phải là số >= 0"})
			return
		}
		current.Price = price
	}
	...
	var p Product
	err = db.QueryRow(`UPDATE products SET name = $1, price = $2, description = $3, image = $4
		WHERE id = $5 RETURNING id, name, price, description, image, created_at`,
...
func deleteProduct(c *gin.Context) {
	var image string
	err := db.QueryRow(`DELETE FROM products WHERE id = $1 RETURNING image`, c.Param("id")).Scan(&image)
	if err == sql.ErrNoRows {
		c.JSON(http.StatusNotFound, gin.H{"message": "Không tìm thấy sản phẩm"})
		return
	}
